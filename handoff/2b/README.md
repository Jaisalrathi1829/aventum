...
| Day 2C onwards — incident injection, anomaly detection, RCA, simulator, agent, frontend | Not started |

Implemented so far: canonical transaction ingestion and an explicitly synthetic payment-infrastructure baseline (gateways, routing, latency, response codes, health) representing **normal operation**. There is no incident injection, no anomaly detection, no RCA, no simulator, no agent, and no frontend.
...
```text
aventum/
├── backend/            # ingestion + synthetic infrastructure, migrations, tests
