...
> **AMENDMENT — Day 3 is implemented.** See [DAY3_IMPLEMENTATION_REPORT.md](DAY3_IMPLEMENTATION_REPORT.md) for measured results. Four schema decisions differ from the sketch below; the concepts and every required field are preserved.
>
> 1. **Ground truth moved to its own table, `incident_ground_truth`**, rather than a flagged column on `incidents`. A flag on a shared row is a convention and conventions leak through an incautious `SELECT *`; a separate table makes the isolation structural, and no diagnosis module imports or names it. Verified by AST scan plus adversarial deleted/corrupted re-runs.
> 2. **`incident_evaluation` split into three tables** — `incident_anomalies` (detection), `incident_hypotheses` (competing explanations with supporting *and* contradicting evidence), `incident_rca_results` (the conclusion). A ranked hypothesis set carrying contradiction cannot be represented honestly in one flat row.
> 3. **Two run-anchor tables added** — `incident_simulation_runs` and `incident_analysis_runs` — mirroring the `ingestion_runs` / `synthetic_generation_runs` pattern, carrying seeds, fingerprints, and stage timings.
> 4. **An eighth evidence type, `confounding_check`, was added** after a measured failure: a degraded gateway drags every dimension intersecting it, and those shadows were inflating rival hypotheses. It tests whether a cohort's anomaly survives excluding the leading suspect on another dimension. See the report's Evidence Model section.
>
> Nine tables total. No existing table was altered. All nine acceptance-gate conditions below are met.
