...
def build_hypotheses(
    ...
    for position, hypothesis in enumerate(hypotheses, start=1):
        ...
        if position == 1 and top_score > 0:
            # Decisiveness matters: a strong hypothesis that barely beats its runner-up
            # deserves less confidence than one that wins outright.
            margin = (top_score - second_score) / top_score
            hypothesis.confidence = round(
                max(0.0, min(1.0, top_score * (0.5 + 0.5 * margin))), 4
            )
        else:
            hypothesis.confidence = round(max(0.0, min(1.0, hypothesis.score * 0.5)), 4)
