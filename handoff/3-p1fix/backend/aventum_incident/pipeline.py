...
def _persist_anomalies(
    ...
    for candidate in detection.candidates:
        row = IncidentAnomaly(
            ...
            alert_role=candidate.alert_role,
            derived_from_cohort_key=candidate.derived_from,
            independence=(
                None if candidate.independence is None else round(candidate.independence, 6)
            ),
        )
        session.add(row)
        session.flush()
    ...
    # Second pass: resolve `derived_from` cohort keys into real anomaly ids now that
    # every row has one. Done here rather than during insert because a derivative can
    # be persisted before the primary that explains it.
    for candidate in detection.candidates:
        if candidate.derived_from is None:
            continue
        parent_id = ids.get(candidate.derived_from)
        if parent_id is not None:
            session.get(IncidentAnomaly, ids[candidate.cohort_key]).derived_from_anomaly_id = (
                parent_id
            )
    session.flush()
    return ids
    ...
    for hypothesis in hypotheses:
        session.add(
            IncidentHypothesis(
                ...
                evidence_strength=round(hypothesis.evidence_strength, 4),
...
def run_analysis(
    ...
    # Counts the ACTIONABLE surface: derivative shadows are recorded but are not
    # independent findings.
    analysis_run.anomalies_found = len(detection.primary_alerts)
