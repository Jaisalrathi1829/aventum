...
# ------------------------------------------------------------------- alert roles
# A single real degradation lights up every cohort that intersects it. Those cohorts
# are statistically genuine -- no significance correction removes them -- but they are
# CAUSALLY DERIVATIVE: their movement disappears once the real cause is excluded.
#
# PRIMARY    -- the anomaly survives excluding every stronger already-primary cohort,
#               so it is standing on its own evidence.
# DERIVATIVE -- the anomaly largely collapses when a stronger cohort is excluded, so it
#               is a shadow of that cohort and must not be presented as an independent
#               actionable alert.
ALERT_ROLE_PRIMARY = "PRIMARY"
ALERT_ROLE_DERIVATIVE = "DERIVATIVE"
ALERT_ROLES = (ALERT_ROLE_PRIMARY, ALERT_ROLE_DERIVATIVE)

# ------------------------------------------------------- confidence calibration
# Anomaly score at which absolute evidence strength saturates. Shares the scale of
# `anomaly_score` (significance x effect), so a cohort at or beyond this is treated as
# carrying full evidential weight.
EVIDENCE_STRENGTH_SATURATION = 12.0
