...
class IncidentAnomaly(Base):
    ...
    # --- causal role (P1-1) ---
    # PRIMARY    -- stands on its own evidence once stronger cohorts are excluded.
    # DERIVATIVE -- statistically real, but its movement is explained by `derived_from`.
    # A derivative alert keeps all of its evidence and stays queryable; it simply must
    # not be presented to an operator (or to Day 4) as an independent actionable cause.
    alert_role: Mapped[str] = mapped_column(Text, nullable=False, server_default="PRIMARY")
    derived_from_cohort_key: Mapped[str | None] = mapped_column(Text, nullable=True)
    derived_from_anomaly_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    # Share of its own movement this cohort retained when the explaining cohort was
    # excluded. NULL when nothing stronger existed to test it against.
    independence: Mapped[object | None] = mapped_column(Numeric(9, 6), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            f"alert_role IN ({_sql_list(ALERT_ROLES)})", name="ck_anomaly_alert_role"
        ),
        # A derivative alert must say what explains it; a primary must not claim a parent.
        CheckConstraint(
            "(alert_role = 'DERIVATIVE') = (derived_from_cohort_key IS NOT NULL)",
            name="ck_anomaly_role_parent_coherent",
        ),
        Index("ix_anomaly_run_rank", "analysis_run_id", "rank"),
        Index("ix_anomaly_run_role", "analysis_run_id", "alert_role"),
...
class IncidentHypothesis(Base):
    ...
    evidence_strength: Mapped[object] = mapped_column(
...
class IncidentRcaResult(Base):
    ...
    # --- action-safety triple (P1-2) ---
    # Confidence answers "how cleanly does the evidence point here"; these answer "how
    # much evidence is there". They are stored separately, never folded together, so a
    # consumer can require BOTH before authorising a larger intervention.
    severity: Mapped[str] = mapped_column(Text, nullable=False, server_default="NONE")
    significance_sigma: Mapped[object] = mapped_column(
