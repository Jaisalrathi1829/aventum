...
def _require_own_ids(ctx: ToolContext, arguments: dict) -> str | None:
    """
    Any `incident_id` / `analysis_run_id` the model supplies must be the one under analysis.

    FOUND BY THE REAL MODEL, AND A GENUINE HOLE.
    qwen3:8b invented `analysis_run_id: 101`. That value flowed into
    `run_counterfactual`, which wrote a simulation row referencing a non-existent
    analysis run — a foreign-key violation that aborted the transaction and poisoned the
    session mid-run. The projection was garbage and the run died on a database error
    rather than a clean refusal.

    A hallucinated ID is now refused before any write. The agent analyses exactly one
    incident; naming a different one is always invalid, so this doubles as a cross-
    incident containment check rather than only an existence check.
    """
    if "incident_id" in arguments:
        supplied = arguments["incident_id"]
        if supplied != ctx.incident_id:
            return (
                f"incident_id {supplied} is not the incident under analysis "
                f"({ctx.incident_id}). Use {ctx.incident_id}."
            )
    if "analysis_run_id" in arguments:
        supplied = arguments["analysis_run_id"]
        if supplied != ctx.analysis_run_id:
            return (
                f"analysis_run_id {supplied} is not the analysis run under analysis "
                f"({ctx.analysis_run_id}). Use {ctx.analysis_run_id}."
            )
    return None
...
def _get_incident_context(ctx: ToolContext, arguments: dict) -> ToolResult:
    run_id = _int_arg(arguments, "analysis_run_id")
    try:
...
def _get_gateway_health(ctx: ToolContext, arguments: dict) -> ToolResult:
    _int_arg(arguments, "incident_id")
...
def dispatch(ctx: ToolContext, tool_name: str, arguments: dict) -> ToolResult:
    ...
    # Reject hallucinated or cross-incident IDs BEFORE any tool can write with them.
    id_error = _require_own_ids(ctx, arguments)
    if id_error is not None:
        return ToolResult(tool_name, OUTCOME_INVALID_REQUEST, error=id_error)
