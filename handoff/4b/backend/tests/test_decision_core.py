...
def test_deterministic_core_contains_no_agent_code():
    """
    The deterministic core stays model-free, asserted rather than assumed.

    AMENDED IN DAY 4B, AND DELIBERATELY NARROWED RATHER THAN DELETED.
    The original assertion included `not (backend / "aventum_agent").exists()`, which
    expressed the Day 4A scope boundary: no agent code had been written yet. Day 4B
    legitimately creates that package, so that one clause is obsolete — but the
    invariant underneath it is not, and is retained and STRENGTHENED below.

    What still matters, and is enforced here:
      * the three Day 4A packages contain no Ollama/Qwen client code, and
      * they do not IMPORT `aventum_agent` at all.

    The second clause is the stronger property and the one that actually protects the
    architecture: it is what keeps Qwen *optional*. If the deterministic spine ever
    imported the agent layer, `AGENT_UNAVAILABLE` could no longer degrade cleanly and
    Day 4A would stop being able to run on its own. Dependency points one way only:
    the agent may call the core; the core may never call the agent.
    """
    import pathlib
    ...
    for package in ("aventum_counterfactual", "aventum_policy", "aventum_action"):
        for module in sorted((backend / package).glob("*.py")):
            ...
            for banned in ("import aventum_agent", "from aventum_agent"):
                assert banned not in source, (
                    f"{module.name} imports the agent layer; the deterministic core must "
                    "never depend on Qwen being present"
                )
