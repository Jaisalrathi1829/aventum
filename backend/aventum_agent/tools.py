...
class ToolContext:
    ...
    # Set by the loop once the AgentRun row exists. It cannot be a constructor argument
    # because the context is built before the run is persisted -- and it must be carried
    # here, because `_propose_action` is the only place that knows a recommendation was
    # authored by the agent rather than by the deterministic pipeline.
...
def _propose_action(ctx: ToolContext, arguments: dict) -> ToolResult:
    ...
    try:
        result = build_recommendation(
            ...
            # Provenance, not decoration: without this the row records NULL and the UI
            # reports an agent-authored recommendation as "deterministic" while the same
            # payload carries rationale_truth = AI_GENERATED. One object cannot claim
            # both that no agent was involved and that its text is AI-generated.
